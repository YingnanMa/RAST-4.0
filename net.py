...
class Net(nn.Module):
    ...
    def forward(self, content, style, batch_size):
        ...
        #output2: leak image
        stylized2 = self.transform(opt1_feats[3], style_feats[3], opt1_feats[4], style_feats[4])
        output2 = self.decoder(stylized2)
        opt2_feats = self.encode_with_intermediate(output2)


        #restoration loss functions right
        content_transitive_loss = self.calc_content_loss(rc1_feats[3], content_feats[3], norm = True) + self.calc_content_loss(rc1_feats[4], content_feats[4], norm = True)
        ...
        #leak loss
        content_leak_loss = self.calc_content_loss(opt1_feats[3], opt2_feats[3], norm = True) + self.calc_content_loss(opt1_feats[4], opt2_feats[4], norm = True)
        ...
        style_transitive_loss2 = self.calc_style_loss(opt2_feats[0], style_feats[0])
        for i in range(1, 5):
            style_transitive_loss2 += self.calc_style_loss(opt2_feats[i], style_feats[i])
        ...
        return output1, output2, rc1, rs1, l_identity1, l_identity2, content_contrastive_loss, style_contrastive_loss, content_transitive_loss, style_transitive_loss, style_diff_loss,content_leak_loss